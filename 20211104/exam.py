class Exam:
    ...
    # 4.
    # 6과 45의 최소공배수를 구하는 코드를 while 루프 기반으로 작성해보자. 참고로 6으로도 나눠지고 45로도 나눠지는 값들 중에서 제일 작은 값이 '최소공배수'이다.
    # 따라서 45부터 시작해서 값을 1씩 증가시켜가면서 6과 45로 나누어 떨어지는 첫 번째 값을 찾으면 된다. 코드는 다음과 같이 작성하자.
    #
